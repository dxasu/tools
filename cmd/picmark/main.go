...
type Config struct {
	...
	Display   bool    `validate:"-"`                   // 是否显示图像方框（调试用）
}
...
func main() {
	var (
		...
		display = flag.Bool("display", false, "显示图像方框（调试用）")
	)
	flag.Parse()
...
func addBox(img image.Image) image.Image {
	// 将原图转换为可写的RGBA图像
	rgbaImg := image.NewRGBA(img.Bounds())
	draw.Draw(rgbaImg, rgbaImg.Bounds(), img, image.Point{}, draw.Src) // 复制原图数据

	width, height := rgbaImg.Bounds().Dx(), rgbaImg.Bounds().Dy()
	for y := 0; y < height; y++ {
		for x := 0; x < width; x++ {
			if x < 1 || y < 1 || x >= width-1 || y >= height-1 { // 边缘像素判断
